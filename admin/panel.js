document.addEventListener('DOMContentLoaded', () => {
    ...
    const cargarConfiguracion = async () => {
        try {
            const response = await fetch(`${API_URL}leer_configuracion.php`);
            const config = await response.json();
            if (config.dias_minimos_alquiler) {
                document.getElementById('dias-minimos-input').value = config.dias_minimos_alquiler;
            }
        } catch (error) {
    ...
    const guardarConfiguracion = async () => {
        const diasMinimos = document.getElementById('dias-minimos-input').value;
        const settings = {
            dias_minimos_alquiler: diasMinimos
        };
        try {
            const response = await fetch(`${API_URL}actualizar_configuracion.php`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    document.getElementById('guardar-config-btn').addEventListener('click', guardarConfiguracion);

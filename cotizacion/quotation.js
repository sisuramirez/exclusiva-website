...
function initializeQuotationTool() {
  const cars = [
      { id: 1, name: "Toyota Hiace", category: "Microbuses", imageUrl: "./img/hiace.png?v=2", price: 80, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Manual" } },
      { id: 2, name: "Nissan Urvan", category: "Microbuses", imageUrl: "./img/urvan.png?v=2", price: 80, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Manual" } },
      { id: 3, name: "Hyundai H1", category: "Microbuses", imageUrl: "./img/h1.png?v=2", price: 70, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Automático/Manual" } },
      { id: 4, name: "Hyundai Staria", category: "Microbuses", imageUrl: "./img/staria.png?v=2", price: 80, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Automático/Manual" } },
      { id: 5, name: "Toyota Fortuner", category: "SUVs", imageUrl: "./img/fortuner.png?v=2", price: 95, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Automático" } },
      { id: 6, name: "Mitsubishi Montero", category: "SUVs", imageUrl: "./img/montero.png?v=2", price: 80, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Automático" } },
      { id: 7, name: "Hyundai Santa Fe", category: "SUVs", imageUrl: "./img/santafe.png?v=2", price: 65, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 8, name: "Mitsubishi Outlander", category: "SUVs", imageUrl: "./img/outlander.png?v=2", price: 65, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 9, name: "Toyota Hilux", category: "Pick-ups", imageUrl: "./img/hilux.png?v=2", price: 75, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Automático/Manual" } },
      { id: 10, name: "Mitsubishi L200", category: "Pick-ups", imageUrl: "./img/l200.png?v=2", price: 75, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Manual/Automático" } },
      { id: 11, name: "Nissan Frontier", category: "Pick-ups", imageUrl: "./img/frontier.png?v=2", price: 75, specs: { airConditioner: "Sí", fuel: "Diésel", transmission: "Automático/Manual" } },
      { id: 12, name: "Hyundai Creta", category: "Crossovers", imageUrl: "./img/creta.png?v=2", price: 55, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 13, name: "Nissan Kicks", category: "Crossovers", imageUrl: "./img/kicks.png?v=2", price: 45, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 14, name: "Kia Sonet", category: "Crossovers", imageUrl: "./img/sonet.png?v=2", price: 45, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 15, name: "Mitsubishi Xpander", category: "Crossovers", imageUrl: "./img/xpander-v2.png?v=2", price: 55, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 16, name: "Kia Rio", category: "Sedanes", imageUrl: "./img/rio.png?v=2", price: 35, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 17, name: "Hyundai Verna", category: "Sedanes", imageUrl: "./img/verna.png?v=2", price: 35, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } },
      { id: 18, name: "Toyota Corolla", category: "Sedanes", imageUrl: "./img/corolla.png?v=3", price: 35, specs: { airConditioner: "Sí", fuel: "Gasolina", transmission: "Automático" } }
  ];
      ...
      selectedCarInfo.innerHTML = `
          ...
          <div class="quotation__car-details">
              ...
              <div class="quotation__car-specs">
                  ...
                  <div class="quotation__car-spec"><span class="car-card__spec-icon">💰</span><span class="quotation__car-spec-text">Precio desde: ${formatCurrency(selectedCar.price)}/dia</span></div>
              </div>
          </div>`;
  ...
  function calculateAndDisplayQuote() {
      ...
      if (extraHours > 0 && extraHoursCost >= dailyPrice) {
          ...
          summaryHTML = `<h4>Resumen de la Cotización</h4><p>Días de renta: <strong>${finalDays}</strong></p><p>Precio por día: ${formatCurrency(dailyPrice)}</p><hr><p class="quotation__total">Subtotal Estimado: <strong>${formatCurrency(finalTotal)}</strong></p>`;
          currentQuoteDetails.rentalDays = finalDays;
      } else {
          finalTotal = (fullDays * dailyPrice) + extraHoursCost;
          summaryHTML = `<h4>Resumen de la Cotización</h4><p>${fullDays} día(s) x ${formatCurrency(dailyPrice)}/día = <strong>${formatCurrency(fullDays * dailyPrice)}</strong></p>${extraHours > 0 ? `<p>${extraHours} hora(s) extra x ${formatCurrency(extraHourPrice)}/hora = <strong>${formatCurrency(extraHoursCost)}</strong></p>` : ''}<hr><p class="quotation__total">Subtotal Estimado: <strong>${formatCurrency(finalTotal)}</strong></p>`;
      ...
      quotationResult.style.display = 'block';
      document.getElementById('proceed-to-form-btn').addEventListener('click', () => { if (calculateAndDisplayQuote()) { showCustomerForm(); } });
      return true;
  }
  ...
  const endDatePicker = flatpickr("#end-date", { ...datePickerConfig, minDate: new Date().fp_incr(2) });
  flatpickr("#start-date", { ...datePickerConfig, onChange: function (selectedDates) { if (selectedDates[0]) { const minEndDate = new Date(selectedDates[0]); minEndDate.setDate(minEndDate.getDate() + 2); endDatePicker.set("minDate", minEndDate); } quotationResult.style.display = 'none'; } });
  ...
  function renderInsuranceAddOns() { if (!addonsContainer) return; addonsContainer.innerHTML = ''; insuranceAddOns.forEach(addon => { const button = document.createElement('button'); button.type = 'button'; button.className = 'addon-button'; button.dataset.id = addon.id; if (addon.selected) button.classList.add('selected'); if (addon.mandatory) button.classList.add('mandatory'); let buttonContent = ''; if (addon.id === 'cdw') { buttonContent += '<p class="basic-insurance-inline-label">Seguro Básico</p>'; } buttonContent += `<h4>${addon.name}</h4><p>${addon.description}</p><p><strong>${formatCurrency(addon.dailyCost)} / día</strong></p>`; if (addon.mandatory) { buttonContent += '<span class="mandatory-label">(Obligatorio)</span>'; } buttonContent += `<button type="button" class="addon-info-button" data-description="${addon.popupDescription}"><img src="./img/question-icon.png" alt="Información" class="addon-info-icon"></button>`; button.innerHTML = buttonContent; addonsContainer.appendChild(button); }); }
  function updateQuoteSummaryAndTotal() { if (!summaryQuoteInfo || !finalTotalContainer || !currentQuoteDetails.baseTotal) return; let addonsTotal = 0; let addonsHTML = '<hr><h4>Seguros y Extras</h4>'; insuranceAddOns.forEach(addon => { if (addon.selected) { const addonCost = addon.dailyCost * currentQuoteDetails.rentalDays; addonsTotal += addonCost; addonsHTML += `<p>${addon.name}: <strong>${formatCurrency(addonCost)}</strong> <br> (${currentQuoteDetails.rentalDays} día(s) x ${formatCurrency(addon.dailyCost)})</p>`; } }); const finalTotal = currentQuoteDetails.baseTotal + addonsTotal; summaryQuoteInfo.innerHTML = currentQuoteDetails.summaryHTML + addonsHTML; finalTotalContainer.innerHTML = `<hr><span class="total-label">Total Final Estimado:</span><strong class="total-amount">${formatCurrency(finalTotal)}</strong>`; }
  ...
  const quotationResultContainer = document.querySelector('#quotation-result'); if (quotationResultContainer) { quotationResultContainer.addEventListener('click', function (event) { if (event.target && event.target.id === 'proceed-to-form-btn') { if (calculateAndDisplayQuote()) { showCustomerForm(); } } }); }
}